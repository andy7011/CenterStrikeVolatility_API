...
def get_object_from_json_endpoint_with_retry(url, method='GET', params={}, max_delay=180, timeout=10):
    ...
    while True:
        try:
            ...
            print(
                f"{datetime.now().strftime('%Y-%m-%d %H:%M:%S')} Запрос к {url} успешно выполнен (строка: {line_number})")
...
def update_output_smile(value, n):
    try:
        ...
        dff_MyPosOrders.loc[:, numeric_columns] = dff_MyPosOrders[numeric_columns].round(2)

        # Преобразуем типы опционов
        ...
        for i in dff_MyPosOrders['_last_price_timestamp']:
            if not isnan(i):  # Проверяем, что значение не NaN
                ...
                dff_MyPosOrders.loc[mask, 'converted_time'] = MSK_time_str
