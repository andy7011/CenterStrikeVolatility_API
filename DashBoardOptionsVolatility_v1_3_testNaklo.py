...
app.layout = html.Div(children=[
    ...
    html.Div(children=[
        ...
        # dcc.Graph(id='MyTilt_history'),

        # График истории "Наклон по всем инструментам"
...
def update_output_history_naklon(dropdown_value, slider_value, radiobutton_value, n):
    ...
    with open(temp_obj.substitute(name_file='OptionsSmileNaklonHistory.csv'), 'r') as file:
        ...
        df_vol_history_naklon['DateTime'] = pd.to_datetime(df_vol_history_naklon['DateTime'], format='%Y-%m-%d %H:%M:%S')
    ...
    # График истории цены базового актива
    fig.add_trace(go.Scatter(x=df_BaseAssetPrice['DateTime'], y=df_BaseAssetPrice['last_price'], mode='lines+text', visible='legendonly',
                             name=dropdown_value, line=dict(color='gray', width=3, dash='dashdot')),
                            secondary_y=False,)
