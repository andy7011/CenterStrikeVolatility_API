...
def BarsHistoryStream():
    """Получает историю баров для указанного инструмента и временного интервала"""
    # Используем default_broker
    broker = default_broker
    time_frame = 'M5'
    # Запрашиваем глубину истории 140 дней
    date_140_days_ago = datetime.now() - timedelta(days=140)
    # Пробегаемся по всем фьючерсам в списке app.supported_base_asset.MAP
    for symbol in iter(MAP):
        print(symbol)
        dataname = f'SPBFUT.{symbol}'
        symbol = broker.get_symbol_by_dataname(dataname)  # Тикер по названию
        bars = broker.get_history(symbol, time_frame, dt_from=date_140_days_ago)  # Получаем всю историю тикера
        print(bars[0])  # Первый бар
        print(bars[-1])  # Последний бар
        print(bars_to_df(bars))  # Все бары в pandas DataFrame
        time.sleep(3)  # Пауза в 3 секундs
...
def _on_trade_impl(data):
    ...
    if trade_data.get('data').get('class_code') == 'SPBOPT':
        ...
        # Создаем DataFrame и добавляем сделку в CSV файл
        ...
        df_trade_quik.to_csv(temp_obj.substitute(name_file='QUIK_Stream_Trades_ALL.csv'), mode='a', sep=';',
                             index=False,
                             header=False)
