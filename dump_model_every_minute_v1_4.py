...
print(f"Очистка файла BaseAssetPriceHistoryDamp.csv от старых записей (старше 14 дней) завершена")
clean_old_records(temp_obj.substitute(name_file='OptionsVolaHistoryDamp.csv'))
print(f"Очистка файла OptionsVolaHistoryDamp.csv от старых записей (старше 14 дней) завершена")
...
def my_function():
    ...
    try:
        ...
        option_list = model_from_api[1] # Опционы
