...
def handle_all_buttons(n_clicks_save, n_clicks_start, n_clicks_stop):
    # Инициализация значений по умолчанию
    ...
    if n_clicks_save is not None and n_clicks_save > 0:
        # Сохранение значений (все кроме running)
        update_config_values(dataname_buy, dataname_sell, expected_profit,
                             Lot_count, Basket_size)
        ...
        # Запуск процесса
        update_config_values(None, None, None, None, None, None, True)
        ...
        # Остановка процесса
        update_config_values(None, None, None, None, None, None, False)
        save_status = "Процесс остановлен"
        button1_style = {'backgroundColor': 'rgb(30, 30, 30)', 'color': 'white', 'display': 'block', 'margin': '0 auto'}
        button2_style = {'backgroundColor': 'rgb(30, 30, 30)', 'color': 'white', 'display': 'block', 'margin': '0 auto'}
        button3_style = {'backgroundColor': 'red', 'color': 'white', 'display': 'block', 'margin': '0 auto'}
    print(dataname_buy, dataname_sell, expected_profit,
                             Lot_count, Basket_size, Timeout, running)
    return [
        button1_style,
        button2_style,
        button3_style,
...
def updateMyQuoteRobot(n, value):
    ...
    # Добавление строки итогов к данным
    df_pos_with_total_ = pd.concat([df_pos, pd.DataFrame([total_row])], ignore_index=True)

    return df_pos_with_total_.to_dict('records')
