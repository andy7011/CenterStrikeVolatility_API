...
def my_function():
    ...
    try:
        ...
        for option in option_list:
            ...
            if option['_strike'] == strikes_map_up[base_asset_ticker] and option['_type'] == 'C': # фильтрация опционов по страйкам
            ...
            elif option['_strike'] == strikes_map_down[base_asset_ticker] and option['_type'] == 'P': # фильтрация опционов по страйкам
                option['datetime'] = current_datetime
                date_object = datetime.strptime(option['_expiration_datetime'], "%a, %d %b %Y %H:%M:%S GMT").date()
                option['_expiration_datetime'] = date_object.strftime('%Y-%m-%d')
        ...
        with open(temp_obj.substitute(name_file='OptionsSmileNaklonHistory.csv'), 'a', newline='') as f:
            ...
            for option_up, option_down in zip(filtered_option_list_up, filtered_option_list_down):
                current_DateTimestamp = datetime.now()
                currentTimestamp = int(datetime.timestamp(current_DateTimestamp))

                if option_up['_last_price_timestamp'] is not None and currentTimestamp - option_up[
                    ...
                    if option_up['_ask_iv'] is None or option_up['_bid_iv'] is None:
                        ...
                        if option_up['_ask_iv'] is not None and option_up['_bid_iv'] is not None and \
                            ...
                            if option_up['_ask_iv'] < option_up['_volatility'] and option_up['_bid_iv'] < option_up['_volatility'] \
                                    or option_up['_volatility'] < option_up['_bid_iv']:
                                Real_vol_up = (option_up['_ask_iv'] + option_up['_bid_iv']) / 2
                                if Real_vol_up > option_up['_volatility'] * 2 or Real_vol_up < option_up['_volatility'] / 2:
                                    Real_vol_up = option_up['_volatility']
                ...
                if option_down['_last_price_timestamp'] is not None and currentTimestamp - option_down[
                    '_last_price_timestamp'] < last_price_lifetime:
                    ...
                    if option_down['_ask_iv'] is None or option_down['_bid_iv'] is None:
                        ...
                        if option_down['_ask_iv'] is not None and option_down['_bid_iv'] is not None and \
                            ...
                            if option_down['_ask_iv'] < option_down['_volatility'] and option_down['_bid_iv'] < option_down['_volatility'] \
                                    or option_down['_volatility'] < option_down['_bid_iv']:
                                Real_vol_down = (option_down['_ask_iv'] + option_down['_bid_iv']) / 2
                                if Real_vol_down > option_down['_volatility'] * 2 or Real_vol_down < option_down['_volatility'] / 2:
                                    Real_vol_down = option_down['_volatility']
                ...
                Quik = option_up['_volatility'] - option_down['_volatility']
                print(f"Real, Quik: {round(Real, 2), round(Quik, 2)}")

                data_options_naklon = [current_DateTimestamp.strftime('%Y-%m-%d %H:%M:%S'),
                                     option_up['_expiration_datetime'], option_up['_base_asset_ticker'], round(Real, 2), round(Quik, 2)]
